"""
chefOST onboarding — SAM2 video segmentation on Modal, logged to W&B.
...
Why this is the optimal shape:
  ...
  * The W&B API key is injected via a Modal Secret -> no interactive login,
...
Run:
    modal run modal_sam2_wandb.py --video cooking_clip.mp4
"""

import modal

app = modal.App("chefost-sam2-wandb")

# Entity and project match your W&B workspace (override entity via WANDB_ENTITY env var).
...
sam2_image = (
    ...
    .pip_install("torch", "torchvision", "numpy", "pillow", "wandb")
    .run_commands(
...
@app.function(
    ...
    timeout=900,
    secrets=[modal.Secret.from_name("wandb")],    # injects WANDB_API_KEY
)
def run_sam2(video_bytes: bytes, prompt_point=None, prompt_label=None, log_every: int = 10):
    ...
    # WANDB_API_KEY comes from the Modal Secret, so no login() call is needed.
    entity = os.getenv("WANDB_ENTITY", WANDB_ENTITY)
    run = wandb.init(
        entity=entity,
        project=WANDB_PROJECT,
        name="modal-sam2",
        tags=["sam2", "modal", "onboarding"],
        config={
    ...
    try:
        ...
        with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            for frame_idx, obj_ids, mask_logits in predictor.propagate_in_video(state):
                ...
                overlay = _overlay(frame, mask)
                wandb.log(
                    {
                        f"segmentation/frame_{frame_idx:05d}": wandb.Image(
                            overlay,
                            caption=f"frame {frame_idx}, mask_area={int(mask.sum())} px",
                        ),
    ...
    print(f"Done — view the run at {run.url}")
